...
const WorkOrder = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        description: "",
        created_by: "", // This will be automatically populated with the context user
        assignedTo: "",  // This will also be a user coming from context
        product: "",
    });
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const workOrderResponse = await axios.get(workOrdersApiUrl, {
                    headers: { Authorization: `Bearer ${user?.token}` }, // Add token to the request
                });
                setSavedData(workOrderResponse.data);
    ...
    const handleSave = async () => {
        const { workOrderNumber, workOrderStatus, description, assignedTo, product } = formData;
        ...
        const data = {
            ...
            description,
            created_by: user?.id, // Use the user ID here
            assigned_to: assignedTo, // Use context user or another logic if required
            product: parseInt(product), // Convert product to an integer
        };
    ...
    const fetchWorkOrders = () => {
        axios.get(workOrdersApiUrl, {
            headers: { Authorization: `Bearer ${user?.token}` }, // Fixed template literal for token
        })
    ...
    const handleDelete = async () => {
        try {
            await axios.delete(`${workOrdersApiUrl}${stepToDelete}/`, {  // Fixed template literal syntax
                headers: { Authorization: `Bearer ${user?.token}` },   // Fixed template literal for token
            });
